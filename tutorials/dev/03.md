...
`receiveTransaction(tx)` extracts the message data from the transaction and uses a convenience-function available on `app.browser` to display the transaction in the UI along with the random number included in its message. We use a similar convenience function to insert the UI component template on the page in the `render` function. Finally, note that we are now dividing up the `render` and `attachEvents` functions per our standard conventions.
