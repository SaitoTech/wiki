...
JavaScript is great at manipulating web content, and it clearly works, but notice the 404 error in the tab title, or the flash of red when you run the tutorial. This is because we did *not* include the module in the `core` list of `modules.config.js`. Any module that wants to exist in the browser with a web address should be included in core and we just need to include a web directory inside our module folder to store any assets (like pictures or audio).
