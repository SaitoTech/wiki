...
Modify your application constructor by adding the following functions to the `Tutorial02` class similarly to [Tutorial 1](https://wiki.saito.io/en/tech/tutorials/01).

Notice the addition of a *UI component* that we are calling MainUI, added via `const MainUI = require('./lib/main');`. Saito modules can include a ```/lib``` directory which contains subclasses and template files.

We create this component in our constructor and update our `render()` function to make this *UI component* to render itself to the screen:
