...
All UI components should contain functions to ```render``` and ```attachEvents```. We could directly call ```this.ui.render()```, but ```super.render()``` will loop through all the ui components we add and render them. 
