...
Many impossibility results in mechanism design rely on the Revelation Principle, introduced by Roger Myerson in his foundational work on mechanism design (Myerson, 1979; 1981). Informally, the Revelation Principle states that for a broad class of mechanisms, any outcome that can be implemented through "indirect mechanisms" that ask participants to take actions within the mechanism can be implemented by "direct mechanisms" in which participants simply report their preferences truthfully and the mechanism computes the outcome.

In situations where the Revelation Principle can be used, analyzing direct mechanisms provides a short-cut for proving the general impossibility of achieving certain outcomes involving efficiency, incentive compatibility, and welfare—optimality. But not every mechanism can be converted to a direct mechanism in theory. While exceptions are rare, they do exist and are not bound by impossibility results generated by revelation-equivalent reasoning.

Saito is explicitly non-convertible under the Revelation Principle. This is an important and valuable property of the mechanism, as many impossibility results which apply to blockchain mechanisms only apply to revelation-equivalent mechanisms. Readers should note here that we are not arguing the Revelation Principle is incorrect, merely that its assumptions do not hold in the setting in which Saito operates as revelation-based reductions are not possible with routing mechanisms.
