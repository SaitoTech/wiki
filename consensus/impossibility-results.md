...
Because transaction fees are non-scalar, they lack the property of monotonicity these models assert they must have. Worse, while these papers acknowledge that other types of utility exist (they recognize that agents have incentives to collude) they never ask users or producers to reveal their valuations for these other forms of utility. This throws the Roughgarden and Shi models into open contradiction: they use the framework for incentive compatibility provided by Maskin (2002) but fail to follow the basic requirements for using it. 
...
Saito Consensus shows why a combinatorial auction is needed to achieve incentive compatibility: users can be motivated to bid honestly only if competition to purchase blockspace exists from producers: if bidding too low triggers producers to switch from being sellers to buyers of blockspace and out-bid them in their own demand-side auction. The prohibition on purchasing blockspace axiomatically prevents this by prohibiting producers from purchasing blockspace, forgetting that producers in combinatorial auctions are not necessarily buying the same bundle of utility as users. And so the solution emerges exactly because the transaction fee is non-scalar and the consensus mechanism is a combinatorial auction.

On a closing note, we observe that all of the TFM papers to date also fail to properly specify the social choice rule with which they desire incentive compatibility. While the authors of most papers implicitly adopt the rule of the Vickrey auction ("efficient allocation") none seem aware this rule cannot be implemented in mechanisms where the supply of utility changes without the problem of "interdependent valuations" breaking the assumption that even scalar fees could be theoretically truthful.
